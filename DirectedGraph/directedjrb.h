...
Jval key; // id
Jval val; // cây đỏ đen đỉnh kề: đỉnh hút
int visited; // kiểm tra xem đỉnh được thăm chưa
Jval back; // Lưu đỉnh liền trước khi duyệt 
int component; // Thành phần liên thông số mấy? Chính là ccnum trong slide
int pre, post; // Số pre. post
Jval weightver; //trọng số của đỉnh
int size; //số đỉnh kề
int dist;
int pijority;
Jval label; // nhãn của đỉnh
int marry; // Stablematching
} *JRB;
